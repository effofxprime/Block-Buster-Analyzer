...
def process_block(height, endpoint_type, endpoint_url):
    if shutdown_event.is_set():
        return None
    try:
        size = np.random.uniform(0, 10)
        timestamp = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
        return height, size, timestamp
...
def generate_scatter_chart(times, sizes, colors, output_image_file_base, lower_height, upper_height):
    ...
    legend_patches = [
        mpatches.Patch(color=py_color_green, label='Less than 1MB'),
        mpatches.Patch(color=py_color_yellow, label='1MB to 2MB'),
        mpatches.Patch(color=py_color_orange, label='2MB to 3MB'),
        mpatches.Patch(color=py_color_red, label='3MB to 5MB'),
        mpatches.Patch(color=py_color_magenta, label='Greater than 5MB')
    ]
...
def generate_heatmap_with_additional_dimensions(times, sizes, output_image_file_base):
    ...
    data["hour"] = pd.to_datetime(data["times"]).dt.hour
    data["day_of_week"] = pd.to_datetime(data["times"]).dt.dayofweek
...
def main():
    ...
    if os.path.exists(json_file_path):
        ...
        lower_height = min(block["height"] for block in block_data)
        upper_height = max(block["height"] for block in block_data)
    else:
